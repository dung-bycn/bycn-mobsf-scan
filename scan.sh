...
if [ -z "$AUTH" ]; then
  echo "AUTH is required to run MobSF action. (AUTH = $AUTH)"
  exit 126
fi
...
RESPONSE=$(curl -F file=@${INPUT_FILE_NAME} ${MOBSF_URL}/api/v1/upload -H "X-Mobsf-Api-Key:${MOBSF_API_KEY}" -H "Authorization:${AUTH}")
...
curl -X POST --url ${MOBSF_URL}/api/v1/scan --data "scan_type=${SCAN_TYPE}&file_name=${FILE_NAME}&hash=${HASH}" -H "Authorization:${AUTH}" -H "X-Mobsf-Api-Key:${MOBSF_API_KEY}" -s -S --output tmp.json
echo "[/api/v1/scan] Scan finisehd"

# Generate the json report.
echo "[/api/v1/report_json] Generate the json report"
curl -X POST --url ${MOBSF_URL}/api/v1/report_json --data "hash=${HASH}" -H "Authorization:${AUTH}" -H "X-Mobsf-Api-Key:${MOBSF_API_KEY}" --output ${OUTPUT_FILE_NAME}.json -s -S
echo "[/api/v1/report_json] JSON report generated"

# Generate the pdf report.
echo "[/api/v1/download_pdf] Generate the PDF report"
curl -X POST --url ${MOBSF_URL}/api/v1/download_pdf --data "hash=${HASH}" -H "Authorization:${AUTH}" -H "X-Mobsf-Api-Key:${MOBSF_API_KEY}" --output ${OUTPUT_FILE_NAME}.pdf -s -S
